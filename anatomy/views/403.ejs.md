...
Il s'agit de la page "403: Accès Interdit" par défaut. Les agents web qui n'acceptent pas HTML verront une version JSON à la place. Vous pouvez personnaliser la logique de contrôle pour vos besoins dans `config / 403.js`. 

Vous pouvez déclencher cette réponse à partir de l'un de vos contrôleurs ou politiques avec : `Return res.forbidden (msg);` (où `msg` est un message d'erreur optionnel à inclure dans la réponse).

<docmeta name="displayName" value="403.ejs">
