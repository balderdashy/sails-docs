...
* **Custom `.toJSON()`** instance methods are no longer supported.  Instead, add a `customToJSON` method to the model class (i.e. outside of the `attributes` dictionary).  See the [model settings documentation](http://sailsjs.com/documentation/concepts/models-and-orm/model-settings) for more info.
