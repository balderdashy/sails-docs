...
Enquanto Sails adere a filosofia de [Convensão-sobre-configuração](http://en.wikipedia.org/wiki/Convention_over_configuration), é importante entender como customizar os padrões de vez em quando. Para quase todas convensões no Sails, há uma serie de opções de configurações que lhe permitem ajustar ou sobrescrever "coisas" para ajustar as suas necessidades. Esta seção da documentação inclui uma referência completa das opções de configurações disponíveis no Sails.

Aplicações Sails podem ter sua [configuração programada](https://github.com/mikermcneil/sails-generate-new-but-like-express/blob/master/templates/app.js#L15), através da especificação de [variáveis de ambiente](https://github.com/mikermcneil/sails-generate-new-but-like-express/blob/master/templates/app.js#L15) ou linhas de comando, mudando o arquivo local ou global [`.sailsrc` files](http://sailsjs.org/documentation/anatomy/myApp/sailsrc.html) ou (melhor ainda) usando o arquivo de configurações convencional localizado na pasta [`config/`](http://sailsjs.org/documentation/anatomy/myApp/config) dos novos projetos. As configurações a serem usadas no seu app estarão disponível em runtime na global `sails.config` que pode ser acessada de qualquer lugar no `sails`.
