...
| `cookie` | ((dictionary)) | _see [express-session#cookie](https://github.com/expressjs/session#cookie)_ | Options for the session cookie.  See the [express-session docs](https://github.com/expressjs/session#cookie) for more info.
| `routesDisabled` | ((array)) | `[]` | An array of route address strings for which built-in session support will be skipped.  Useful for performance tuning; particularly preventing the unnecessary creation of sessions in requests for assets (e.g. `['GET /js/*', 'GET /styles/*', 'GET /images/*']`).
