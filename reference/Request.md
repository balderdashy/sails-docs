...
#### req.socket.id
 The `id` of the current socket.  This is generated by the socket.io server when a client first connects, and is a valid unique identifier until the socket is disconnected (e.g. if the client is a web browser, until the user closes her browser tab.)

#### Other methods and properties
Sails provides direct, low-level access to the Socket.io methods and properties on `req.socket`, including `req.socket.join`, `req.socket.leave`, and `req.socket.broadcast`. See the relevant docs in the [Socket.io wiki](https://github.com/LearnBoost/socket.io/wiki/Rooms) for more information.
...
The URL path this request was sent. original URL string requested by the user-agent. 
