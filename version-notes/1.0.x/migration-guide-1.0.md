...
* **The `methodOverride` middleware has been removed** -- if your app utilizes this middleware, simply `npm install --save method-override`, make sure your `sails.config.http.middleware.order` array (in `config/http.js`) includes `methodOverride` and add `methodOverride: require('method-override')()` to `sails.config.http.middleware`.
